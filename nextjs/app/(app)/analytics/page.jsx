...
export default function Analytics() {
  ...
  const modeCount = sessions.reduce((acc, s) => { acc[s.mode] = (acc[s.mode] || 0) + 1; return acc }, {})
  const modeData = [
  ...
  const avgScore = sessions.length ? Math.round(sessions.reduce((s, r) => s + r.score, 0) / sessions.length) : 0
  ...
  return (
    <div className="page analytics-page">
      ...
      <div className="stats-row">
        ...
        <StatCard icon={<img src="/assets/stat_card/Streak.svg" alt="streak" width={28} height={28} />} label="Best Streak" value={profile?.streak || 0} suffix="days" />
        <StatCard icon={<Target size={28} />} label="Avg Score" value={`${avgScore}%`} />
        <StatCard icon={<img src="/assets/stat_card/StudyTime.svg" alt="study time" width={28} height={28} />} label="Study Time" value={`${Math.round((profile?.total_study_time_minutes || 0) / 60 * 10) / 10}h`} />
      </div>

      <div className="analytics-grid">
        <div className="card chart-card-lg">
          <div className="card-header"><h3><TrendingUp size={16} /> XP This Week</h3></div>
        ...
        <div className="card chart-card">
          <div className="card-header"><h3><BookOpen size={16} /> Session Modes</h3></div>
          <ResponsiveContainer width="100%" height={200}>
      ...
      {moduleStats.length > 0 && (
        <div className="card" style={{ marginBottom: '1.5rem' }}>
          <div className="card-header"><h3><Target size={16} /> Module Mastery</h3></div>
          <div className="mastery-list">
            {moduleStats.map((mod, i) => (
              <motion.div key={mod.title} className="mastery-row" initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}>
                <span className="mastery-title">{mod.title}</span>
                <div className="mastery-bar-track">
                  <motion.div className={`mastery-bar-fill mastery-${getMasteryColor(mod.mastery_score)}`} initial={{ width: 0 }} animate={{ width: `${mod.mastery_score}%` }} transition={{ delay: i * 0.05 + 0.3, duration: 0.8 }} />
                </div>
      ...
      <div className="card">
        <div className="card-header"><h3><BookOpen size={16} /> Session History</h3></div>
        {loading ? (
          ...
          <div className="session-table">
            <div className="session-table-header">
              <span>Mode</span><span>Module</span><span>Score</span><span>XP</span><span>Duration</span><span>Date</span>
            </div>
            {sessions.map(s => (

...
import { User, Mail, Flame, Zap, BookOpen, Clock, TreeDeciduous } from 'lucide-react'
...
export default function Profile() {
  ...
  async function saveUsername() {
    ...
    try {
      const { error } = await insforge.database.from('profiles').update({ username: username.trim() }).eq('id', user.id)
  ...
  return (
    <div className="page profile-page">
      ...
      <div className="stats-row">
        ...
        <StatCard icon={<Clock size={18} />}    label="Hours"    value={Math.round((profile?.total_study_time_minutes || 0) / 60 * 10) / 10} color="sky" />
      </div>

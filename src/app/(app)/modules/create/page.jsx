...
export default function ModuleCreate() {
  ...
  const onDrop = useCallback((accepted, rejected) => {
    if (rejected?.[0]) {
      ...
      if (codes.includes('file-too-large')) toast.error('File is too large. Maximum 20 MB allowed.')
      else toast.error(rejected[0].errors?.[0]?.message || 'File not accepted')
      return
    }
    ...
    try {
      ...
      if (file) {
        ...
        try {
          ...
          if (storageErr) throw new Error(storageErr.message || 'Upload failed.')
  ...
  return (
    <div className="page module-create">
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="create" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            ...
            <div className="form-card">
              <div className="field">
                ...
                <input className="input" type="text" placeholder="e.g. Biology Chapter 5 — Cell Division"
                  value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} maxLength={120} />
              </div>
              <div className="field">
                <label>Description</label>
                <input className="input" type="text" placeholder="Brief description (optional)"
                  value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
              </div>
              <div className="field">
                <label>Study Content</label>
                <textarea className="input textarea" placeholder="Paste your notes, lesson text, or study material here..."
                  value={form.content} onChange={e => setForm(f => ({ ...f, content: e.target.value }))} rows={8} />
                ...
                <div {...getRootProps()} className={`dropzone ${isDragActive ? 'active' : ''} ${file ? 'has-file' : ''}`}>
                  ...
                  {file ? (
                    <div className="file-preview">
                      <FileText size={18} />
                      <span>{file.name}</span>
                      <button type="button" onClick={e => { e.stopPropagation(); setFile(null) }}><X size={13} /></button>
                    </div>
                  ) : (
              ...
              <div className="field">
                ...
                <div className="tag-input-row">
                  <input className="input" type="text" placeholder="Add a tag..." value={tagInput}
                    onChange={e => setTagInput(e.target.value)}
                    onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addTag())} />
                  <button type="button" className="btn-ghost btn-sm" onClick={addTag}><Plus size={15} /></button>
                </div>
                <div className="tags-row">
                  {form.tags.map(t => (
                    <span key={t} className="tag"><Tag size={10} /> {t}<button type="button" onClick={() => removeTag(t)}><X size={10} /></button></span>
                  ))}
                </div>
              </div>

              <motion.button className="btn-primary btn-lg" onClick={handleSave} disabled={saving || uploading} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                {saving || uploading ? <><span className="spinner" /> {uploading ? 'Uploading...' : 'Saving...'}</> : <><Sparkles size={17} /> Create & Generate AI Questions</>}
              </motion.button>
            </div>
          </motion.div>
        )}
        ...
        {step === 'generating' && (
          <motion.div key="generating" className="generating-screen" initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
            ...
            <div className="generating-steps">
              {GEN_STEPS.map((s, i) => (
                <motion.div key={s} className="gen-step" initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.45 }}>
        ...
        {step === 'done' && (
          <motion.div key="done" className="done-screen" initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }}>
            <motion.div className="done-check" animate={{ scale: [0.9, 1.08, 1] }} transition={{ duration: 0.5 }}>
            ...
            <div className="done-buttons">
              <button className="btn-primary" onClick={() => router.push(`/modules/${moduleId}`)}>Start Studying</button>

...
export default function Analytics() {
  ...
  const modeData = [
    { name: 'Flashcard 🃏', value: modeCount.flashcard || 0, fill: 'var(--emerald)' },
  ...
  return (
    <div className="page analytics-page">
      ...
      <div className="stats-row">
        ...
        <StatCard icon={<Flame />} label="Best Streak" value={profile?.streak || 0} suffix="days" color="amber" />
        <StatCard icon={<Target />} label="Avg Score" value={`${avgScore}%`} color="emerald" />
        <StatCard icon={<Clock />} label="Study Time" value={`${Math.round((profile?.total_study_time_minutes || 0) / 60 * 10) / 10}h`} color="sky" />
      </div>
      ...
      <div className="analytics-grid">
        ...
        <div className="card chart-card-lg">
          <div className="card-header">
            <h3><TrendingUp size={18} /> XP This Week</h3>
          </div>
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={weeklyXp}>
              ...
              />
              <Area type="monotone" dataKey="xp_earned" stroke="var(--emerald)" fill="url(#xpGrad)" strokeWidth={2.5} name="XP" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="card chart-card">
          <div className="card-header">
            <h3><BookOpen size={18} /> Session Modes</h3>
          </div>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={modeData} barSize={28}>
              <XAxis dataKey="name" tick={{ fontSize: 11, fill: 'var(--text-muted)' }} axisLine={false} tickLine={false} />
              <YAxis hide />
              <Tooltip contentStyle={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '8px', fontSize: 12 }} />
      ...
      {moduleStats.length > 0 && (
        <div className="card">
          <div className="card-header">
            <h3><Target size={18} /> Module Mastery</h3>
          </div>
      ...
      <div className="card">
        <div className="card-header">
          <h3><BookOpen size={18} /> Session History</h3>
        </div>
        {loading ? (
          ...
          <div className="session-table">
            ...
            {sessions.map(s => (
              <div key={s.id} className="session-table-row">
                <span className="mode-badge">{modeLabel(s.mode)}</span>
                ...
                <span className="date-cell">{formatShortDate(s.completed_at)}</span>
              </div>
            ))}
          </div>
        )}
...
function modeLabel(mode) {
  return { flashcard: '🃏 Flash', mcq: '🎯 MCQ', speed: '⚡ Speed' }[mode] || mode
}

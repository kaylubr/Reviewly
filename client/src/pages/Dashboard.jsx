...
import { Plus, BookOpen, Zap, Target, Flame, TrendingUp, Clock, ChevronRight } from 'lucide-react'
  ...
  return (
    <div className="page dashboard">
      ...
      <div className="dashboard-header">
        <div>
          <h1>Good {getGreeting()}, {profile?.username || 'Learner'} 👋</h1>
      ...
      <div className="stats-row">
        <StatCard icon={<Flame size={20} />} label="Day Streak" value={profile?.streak || 0} suffix="days" color="amber" />
        <StatCard icon={<Zap size={20} />} label="Total XP" value={profile?.xp || 0} color="violet" />
        <StatCard icon={<BookOpen size={20} />} label="Sessions" value={profile?.total_sessions || 0} color="emerald" />
        <StatCard icon={<Clock size={20} />} label="Study Time" value={Math.round((profile?.total_study_time_minutes || 0) / 60 * 10) / 10} suffix="hrs" color="sky" />
      </div>
      ...
      <div className="dashboard-grid">
        ...
        >
          <div className="tree-card-content">
            <div>
              <p className="card-label">Your Knowledge Tree</p>
              <h2 className="tree-level">Level {lvlInfo.level}</h2>
              <p className="tree-stage-name">{treeStage.name}</p>
              <XPBar
                current={lvlInfo.currentLevelXp}
                max={lvlInfo.nextLevelXp}
                level={lvlInfo.level}
              />
            </div>
            <motion.div
            ...
            >
              <span style={{ fontSize: '4rem' }}>{treeStage.emoji}</span>
            </motion.div>
          </div>
        ...
        <div className="card chart-card">
          <div className="card-header">
            <h3><TrendingUp size={18} /> Weekly XP</h3>
          </div>
          <ResponsiveContainer width="100%" height={140}>
            <AreaChart data={weeklyXp}>
              ...
              />
              <Area type="monotone" dataKey="xp_earned" stroke="var(--emerald)" fill="url(#xpGradient)" strokeWidth={2} name="XP" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="section-header">
        <h2>Your Modules</h2>
        <button className="btn-ghost" onClick={() => navigate('/modules/create')}>
          <Plus size={16} /> New
        </button>
      </div>
      ...
      {recentSessions.length > 0 && (
        <>
          <div className="section-header">
            ...
            <button className="btn-ghost" onClick={() => navigate('/analytics')}>View all</button>
          </div>
...
function SessionRow({ session }) {
  const modeIcons = { flashcard: '🃏', mcq: '🎯', speed: '⚡' }
  return (
    <div className="session-row">
      <span className="mode-badge">{modeIcons[session.mode]} {session.mode}</span>
